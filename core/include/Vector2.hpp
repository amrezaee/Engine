...
public:
	...
	constexpr Vector2(A a, B b)
	        : x(static_cast<T>(a)),
	          y(static_cast<T>(b))
	{
	}

	explicit constexpr Vector2(T* p)
	        : x(p[0]),
	          y(p[1])
	{
	}
	...
	explicit constexpr Vector2(A* pa)
	        : x(static_cast<T>(pa[0])),
	          y(static_cast<T>(pa[1]))
	{
	}

	template<typename A>
	constexpr Vector2(const Vector2<A>& v)
	        : Vector2(v.x, v.y)
	{
	}
	...
	// logical operations
	constexpr bool operator==(Vector2<T> r) const
	{
		return x == r.x && y == r.y;
	}
	constexpr bool operator!=(Vector2<T> r) const
	...
	constexpr Vector2<T> operator+(T r) const
	{
